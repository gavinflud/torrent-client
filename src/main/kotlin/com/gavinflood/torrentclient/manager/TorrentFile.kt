...
class TorrentFile(private val path: String, private val charset: Charset = Charset.forName("ASCII")) {
    ...
    private fun populateInfo(data: ByteArray) {
        val map = BDecoder(String(data, charset)).parse()
        ...
        announce = map.getEntry<BString>("announce").value
        val info = map.getEntry<BMap>("info")
        name = info.getEntry<BString>("name").value
        ...
        val piecesByteArray = info.getEntry<BString>("pieces").value.toByteArray(charset)
